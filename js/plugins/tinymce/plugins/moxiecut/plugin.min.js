(function (exports, undefined) {
	...
	var __moxman_util_Loader = "moxman/util/Loader",
	__moxman_Env = "moxman/Env",
	__moxman_util_I18n = "moxman/util/I18n",
	__moxman_Loader = "moxman/Loader",
	__moxman_interop_TinyMcePlugin = "moxman/interop/TinyMcePlugin";
	define(__moxman_util_Loader, [], function () {
		"use strict";
		var idCount = 0,
		loadedUrls = {};
		function noop() {}
		...
		var Loader = {
			...
			loadCss : function (url, loadedCallback, errorCallback) {
				...
				function waitForWebKitLinkLoaded() {
					...
					if ((new Date).getTime() - startTime < Loader.maxLoadTime * 1e3) {
	...
	define(__moxman_Env, [], function () {
		return {
			ie7 : document.all && !window.opera && !document.documentMode
		}
	});
	define(__moxman_util_I18n, [], function () {
		"use strict";
		var itemList = {};
		var data = {};
		return {
	...
	define(__moxman_Loader, [__moxman_util_Loader, __moxman_Env, __moxman_util_I18n], function (ResourceLoader, Env, I18n) {
		...
		function loadAndRender(view) {
			return function (settings) {
				...
				function done() {
					...
					moxman.ui.FloatPanel.zIndex = moxman.ui.FloatPanel.zIndex || settings.zIndex;
					var manager = new moxman.Manager(settings);
					manager.init(new moxman.views[view](manager))
				}
				if (!moxman.ui) {
					ResourceLoader.load({
						js : [Env.baseUrl + "/js/moxman.api.min.js", Env.baseUrl + "/api.php?action=language" + (settings.code ? "&code=" + settings.code : ""), Env.baseUrl + "/api.php?action=PluginJs"],
	...
	define(__moxman_interop_TinyMcePlugin, [__moxman_Loader, __moxman_Env], function (Loader, Env) {
		tinymce.PluginManager.add("moxiecut", function (editor, url) {
			Env.baseUrl = url;
			editor.settings.file_browser_callback = function (id, value, type, win) {
			/* Agregado por MarianoO */
				var settings = {};
				tinymce.each(editor.settings, function (value, key) {
					if (key.indexOf("moxiecut_") === 0) {
				...
				//Loader.browse({
				//	zIndex : editor.windowManager.zIndex || tinymce.ui.FloatPanel && tinymce.ui.FloatPanel.currentZIndex,
				//	url : editor.documentBaseURI.toAbsolute(value),
				//	view : type == "image" ? "thumbs" : "files",
				//	oninsert : function (args) {
				//		var fieldElm = win.document.getElementById(id);
			...
			editor.addCommand("mceInsertFile", function (ui, value) {
				...
				Loader.browse(tinymce.extend(settings, {
						zIndex : zIndex,
						view : editor.getParam("moxiecut_default_view", "files"),
						document_base_url : editor.settings.document_base_url,
						oninsert : function (args) {
							var html = "";
							tinymce.each(args.files, function (file, i) {
								...
								if (isImage) {
									html += processTemplate(editor.getParam("moxiecut_image_template", '<img src="{$meta.url}" ' + 'width="{$meta.width}" height="{$meta.height}">'), file)
								} else {
									html += processTemplate(editor.getParam("moxiecut_file_template", '<a href="{$url}">{$name}</a>'), file)
								}
							});
							selection.setRng(lastRng);
				...
				//Loader.browse(tinymce.extend(settings, {
				//		zIndex : editor.windowManager.zIndex,
				//		view : editor.getParam("moxiecut_default_view", "files"),
				//		oninsert : function (args) {
				//			var html = "";
				//			tinymce.each(args.files, function (file, i) {
			...
			if (tinymce.Env) {
				...
				editor.addButton("insertfile", {
					...
					title : editor.getLang("moxiecut_insert", "Insert file"),
					cmd : "mceInsertFile"
				})
			}
		});
		...
		if (langCode && langCode != "en") {
			tinymce.ScriptLoader.load((tinymce.PluginManager.urls.moxiecut || tinymce.baseURL + "/plugins/moxiecut") + "/api.php?action=language&tinymce=true&code=" + langCode)
		}
	});
	expose([__moxman_util_Loader, __moxman_Env, __moxman_util_I18n, __moxman_Loader, __moxman_interop_TinyMcePlugin])
})(this);