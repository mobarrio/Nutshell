...
$uploaddir     = $_SESSION['APP_DOCUMENT_ROOT'] . "/uploads";
$globaldirs    = array( "."=>array("ext"=>"jpg,gif,png,bmp,pdf"), );
...
function sendparamsfile($path,$size,$modify,$file=false){
    ...
    $data = array(      //для того чтобы автоматом подключало режим thumbnails
        ...
        "exists"=>true,
        "meta"=>array(
			"url"          => $url.$uploaddir.$path,
			//--- Mariano 29.05.2013 11:04
			//--- Propiedades para que funcione la multiple inclusion
			"width"        => "",
			"height"       => "",
			"thumb_url"    => $url.$uploaddir.$path,
			"thumb_width"  => "90px",
			"thumb_height" => "67.5px",
			//
			),
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($method == "FileInfo")
    {
		//$PARAMS["paths"][0] .= $uploaddir . $PARAMS["paths"][0];
        $OUT["jsonrpc"] = $jsrpc;
        //вставка изображения
        if($PARAMS["insert"])
        {
			//--- Mariano 29.05.2013 10:50
			//--- Generar info para cada archivo en el paths Multiple inclusion
			foreach($PARAMS["paths"] as $file)
			{
				...
				if($globaldirs[$patharr[1]] && !strstr($file,".."))
				{
					...
					{
						$attr = stat($workdir.$file);
						$modify =  $attr[9];
						$size = $attr[7];
						$type = strtolower(substr(strrchr($file,"."),1));
						$preview = ($type == "jpg" ? "p" : "-");
						$attr = "-rwr-v".$preview;
						$OUT["result"][] = sendparamsfile($file,$size,$modify,true);
					}
				}
			}
			//---	
